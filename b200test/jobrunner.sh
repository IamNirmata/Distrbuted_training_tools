...
dtools_dir=$hdir/distrbuted_training_tools/
...
# --- config ---
NS="gcr-admin"
TEMPLATE="/home/hari/b200/tmp_dltest_template.yml"   # path to the YAML you pasted (save it once)
NODES_FILE="${hdir:-$HOME}/nodes.txt"                # uses $hdir if you already set it
OUTDIR="${hdir:-$HOME}/job_yamls"                    # where to write per-node yamls
...
kubectl get nodes --no-headers -o custom-columns=NAME:.metadata.name \
  | grep "gpu" > $hdir/nodes.txt

#step 2: Create tmp_job.yml
export gcrjobname=$(kubectl create -f $dltest_dir/dltest.yml \
